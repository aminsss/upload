...
$(".CloseButton").on("click", function ()  { $(".block-slide").hide(); $('body,html').css('overflow', ''); });
$(".back-slide").on("click", function () { $(".block-slide").hide(); $('body,html').css('overflow', ''); }); $(".zoom-kala").on("click", function () { $("body").css("overflow", "hidden"); $(".block-slide").fadeIn(); }); $(document).ready(function () {
    $(".picsscroll img").click(function (event) {
        $("#slidercontent").html(""); $("#slidercontent").prepend('<img class=" slidecontent-image" src="/Content/Upload/productImages/Images/' + $(this).attr('id').replace("preview_", "") + '.jpg" />'); $(".activemode").removeClass("activePic"); $("#preview_" + $(this).attr('id').replace("preview_", "")).addClass("activePic");
         var $container = $('.all-galleries'), $scrollTo = $("#preview_" + $(this).attr('id').replace("preview_", ""));  $container.animate({ scrollTop: $scrollTo.offset().top - $container.offset().top - ((innerHeight - 180) / 2) + $container.scrollTop() }, 400)
    });
})